...
class gdaUtility:
    def __init__(self):
        ...
        self._rangeDict = dict(col1="TBD",
                               coverage=dict(colCountManyRawDb=None, colCountOneRawDb=None,
                                             coveragePerCol="TBD", totalValCntAnonDb=None,
                                             valuesInBothRawAndAnonDb=None))
    ...
    def _getWorkingColumns(self, tabChar, allowedColumns):
        ...
        for col in tabChar:
            if (((tabChar[col]['column_type'] == "real") or (
                    (tabChar[col]['column_type'][:3] == "int"))) and (col in allowedColumns)):
        ...
        tuples = []
        for col in tabChar:
            if (tabChar[col]['column_type'][:4] == "date" and col in allowedColumns):
                tuples.append([col, tabChar[col]['num_distinct_vals']])
        ordered = sorted(tuples, key=lambda t: t[1], reverse=True)
        for i in range(len(ordered)):
            if i >= targetCols:
        ...
        tuples = []
        for col in tabChar:
            if (tabChar[col]['column_type'] == "text" and col in allowedColumns):
                tuples.append([col, tabChar[col]['num_distinct_vals']])
        ordered = sorted(tuples, key=lambda t: t[1], reverse=True)
        for i in range(len(ordered)):
            if i >= targetCols:
    ...
    def _measureAccuracy(self, param, attack, tabChar, table, uid, allowedColumns):
        ...
        for rang in ranges:
            for nc in numColumns:
                cond = []
                q = findQueryConditions(param, attack, columns, allowedColumns, rang[0], rang[1],
                                        numColumns=nc)
                pp.pprint(q)
                while (1):
        ...
        for query in queries:
            if (param['basicConfig']['measureParam'] == "rows"):
                ...
                sql = str(f"SELECT sum({numberTypeCols[0]}) FROM {table} ")
            elif (param['basicConfig']['measureParam'] == "avg"):
                sql = str(f"SELECT avg({numberTypeCols[0]}) FROM {table} ")
            else:
    ...
    def _calAccuracy(self, queries, param):
        ...
        if (param['basicConfig']['measureParam']) == "rows":
            ...
            absDict['compute'] = "(sum((numberTypeCol)rawDb)-sum((numberTypeCol)anonDb))"
        elif (param['basicConfig']['measureParam']) == "avg":
            absDict['compute'] = "(avg((numberTypeCol)rawDb)-avg((numberTypeCol)anonDb))"
        else:
        ...
        if (param['basicConfig']['measureParam']) == "rows":
            ...
            simpleRelDict['compute'] = "(sum(rawDb(numberTypeCol))/sum(anonDb(numberTypeCol)))"
        elif (param['basicConfig']['measureParam']) == "avg":
            simpleRelDict['compute'] = "(avg(rawDb(numberTypeCol))/avg(anonDb(numberTypeCol)))"
        else:
        ...
        if (param['basicConfig']['measureParam']) == "sum":
            relDict['compute'] = ("(abs(sum((numberTypeCol)rawDb)-sum((numberTypeCol)anonDb))/max("
                                  "sum((numberTypeCol)rawDb),sum((numberTypeCol)anonDb)))")
        if (param['basicConfig']['measureParam']) == "avg":
            relDict['compute'] = ("(abs(avg((numberTypeCol)rawDb)-avg((numberTypeCol)anonDb))/max("
                                  "avg((numberTypeCol)rawDb),avg((numberTypeCol)anonDb)))")
        else:
            relDict[
    ...
    def _measureCoverage(self, param, attack, tabChar, table, rawColNames, anonColNames):
    ...
    def _calCoverageSumAvg(self, rawDbrowsDict, anonDbrowsDict, colNames, param):
        ...
        if rawResAvg == 0:
            ...
            coverage['coverage']['coveragePerCol'] = anonResAvg / rawResAvg
        columnParam = {}
        colPos = 1
        for col in colNames:
            columnParam["col" + str(colPos)] = col
    ...
    def _getAllowedColumns(self, coverageScores):
        ...
        for cov in coverageScores:
            if (cov['coverage']['coveragePerCol'] is None or cov['coverage'][
                'coveragePerCol'] > 0.001):
